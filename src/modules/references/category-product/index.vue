<template>
  <div>
    ...
    >
      <template v-slot:top>
        <div class="q-table__title">{{ $t('references.category_product') }}</div>
        <q-space/>
        <vs-button v-if="$can('Save', 'Category')" success animation-type="rotate" @click="createForm">
          <q-icon name="add"/>
          <template #animate>
            <q-icon name="add"/>
          </template>
      ...
      <template v-slot:body-cell-action="props">
        <q-td class="text-center">
          <vs-button v-if="$can('Update', 'Category')" style="display: inline-block" flat icon primary @click="editForm(props.row)">
            <q-icon name="edit"/>
          </vs-button>
          <vs-button v-if="$can('Delete', 'Category')" style="display: inline-block" flat icon danger @click="deleteModalTrue(props.row.id)">
            <q-icon name="delete"/>
          </vs-button>
        </q-td>
      </template>
    ...
    <vs-dialog blur prevent-close v-model="createModal">
      <template #header>
        <b v-if="create">{{ $t('references.new_item_category') }}</b>
        <b v-else>{{ $t('references.edit_item_category') }}</b>
      </template>
      <q-separator/>
...
export default {
  ...
  methods: {
    ...
    deleteGroup() {
      CategoryService.deleteItemCategory(this.categoryId).then(res => {
    ...
    save() {
      this.$refs.categoryForm.validate().then(valid => {
        if (valid) {
          if (this.create) {
            CategoryService.createItemCategory(this.form).then(res => {
            ...
            CategoryService.updateItemCategory(this.form).then(res => {
              this.createModal = false
              this.getCategoryList();
            }).catch(e => {
              this.$vs.notification({
