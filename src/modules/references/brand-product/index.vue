<template>
  <div>
    ...
    >
      <template v-slot:top>
        <div class="q-table__title">{{ $t('references.brand') }}</div>
        <q-space/>
    ...
    <vs-dialog blur prevent-close v-model="createModal">
      <template #header>
        <b v-if="create">{{ $t('references.new_brand') }}</b>
        <b v-else>{{ $t('references.edit_brand') }}</b>
      </template>
      <q-separator/>
...
export default {
  ...
  methods: {
    ...
    deleteGroup() {
      BrandService.deleteBrand(this.categoryId).then(res => {
    ...
    save() {
      this.$refs.categoryForm.validate().then(valid => {
        if (valid) {
          if (this.create) {
            BrandService.createBrand(this.form).then(res => {
            ...
            BrandService.updateBrand(this.form).then(res => {
              this.createModal = false
              this.getBrandsList();
            }).catch(e => {
              this.$vs.notification({
