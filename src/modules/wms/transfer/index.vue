      ...
      >
        <template v-slot:top>
          <div v-if="windowWidth > 600" class="q-table__title">
            ...
            <span v-if="tab === 'received'">{{ $t('wms.transfer.received') }}</span>
            <span v-if="tab === 'pending'">{{ $t('wms.transfer.pending') }}</span>
          </div>
          ...
          <div v-if="windowWidth < 600" class="q-gutter-y-md  mb-1" style="max-width: 300px;">
            <q-tabs v-model="tab" inline-label outside-arrows mobile-arrows narrow-indicator active-color="primary" class="text-primary mr-2">
              ...
              <q-tab :tab="tab" name="received" :label="$t('wms.transfer.received')"></q-tab>
              <q-tab :tab="tab" name="pending" :label="$t('wms.transfer.pending')"></q-tab>
            </q-tabs>
          </div>
      ...
      <template v-if="windowWidth < 600">
        <q-toolbar>
          <vs-row>
            ...
            <vs-input state="primary" icon-after  v-model="search" :placeholder="$t('actions.search')">
              <template #icon>
                <q-icon name="search"/>
              </template>
            </vs-input>
          ...
          <vs-col :w="windowWidth > 600 ? '6' : '12'">
            <validation-provider v-slot="{valid}" rules="required" name="fromDepartmentId">
              <vs-select
                  ...
                  @change="getItemById(formTransfer.fromDepartmentId)"
                  v-model="formTransfer.fromDepartmentId"
                  :key="`from_departmentList-${departmentList.length}`"
              >
                <vs-option-group>
          ...
          <vs-col :w="windowWidth > 600 ? '6' : '12'">
            <validation-provider v-slot="{valid}" rules="required" name="toDepartmentId">
              <vs-select
                  ...
                  :class="!valid ? 'required' : ''"
                  v-model="formTransfer.toDepartmentId"
                  :key="`departmentList2-${departmentList.length}`"
              >
                <vs-option-group>
                  <div>{{ $t('doc.to_department') }}</div>
                  <vs-option v-for="(lol, i) in departmentList" :key="`departmentList2-${i}`" :label="lol.name" :value="lol.id" :disabled="formTransfer.fromDepartmentId == lol.id">
                    {{ lol.name }}
                  </vs-option>
                </vs-option-group>
              </vs-select>
...
<!--          </vs-col>-->
          ...
          <vs-col w="2">
            <vs-button class="mt-3 ml-1" success animation-type="rotate" @click="openAndClose">
              <q-icon name="add"/>
              <template #animate>
                <q-icon name="add"/>
              </template>
          ...
          <vs-row v-if="transferBySomeStuffModel">
            <vs-col :w="windowWidth > 600 ? '3' : '12'">
              <vs-select
                  block
                  class="mt-3"
                  :label="$t('items.brand')"
...
<!--                  <div>{{ $t('items.brand') }}</div>-->
                  <vs-option v-for="(b, i) in brands" :key="`brands-${i}`" :label="b.name" :value="b.id">
                    {{ b.name }}
                  </vs-option>
<!--                </option-group>-->
              </vs-select>
...
<!--                  <div>{{ $t('items.category') }}</div>-->
                  <vs-option v-for="(cat, i) in categories" :key="`categories-${i}`" :label="cat.name" :value="cat.id">
                    {{ cat.name }}
                  </vs-option>
<!--                </option-group>-->
              </vs-select>
            ...
            <vs-col :w="windowWidth > 600 ? '3' : '12'">
              <vs-select
                  ...
                  :label="$t('items.seasons')"
                  v-model="seasonsId"
                  :key="`seasons-${seasons.length}`"
              >
<!--                <option-group>-->
<!--                  <div>{{ $t('items.seasons') }}</div>-->
                  <vs-option v-for="(ses, i) in seasons" :key="`seasons-${i}`" :label="ses.name" :value="ses.id">
                    {{ ses.name }}
                  </vs-option>
<!--                </option-group>-->
              </vs-select>
            </vs-col>
            <vs-col :w="windowWidth > 600 ? '1' : '6'">
              <vs-button class="mt-3" :class="windowWidth > 600 ? 'ml-3' : ''" icon animation-type="rotate" @click="filter">
                <q-icon name="search"/>
              </vs-button>
            </vs-col>
            <vs-col :w="windowWidth > 600 ? '1' : '6'">
              <vs-button color="danger" :class="windowWidth > 600 ? 'ml-1' : ''" class="mt-3" icon animation-type="rotate" @click="clear">
                <q-icon name="delete"/>
              </vs-button>
            </vs-col>
          </vs-row>
...
import BrandService from "@/modules/references/brand-product/brand.service";
import SeasonService from "@/modules/references/seasons/seasons.service";
import CategoryService from "@/modules/references/category-product/category.service";
...
export default {
  ...
  watch: {
    ...
    'search1': {
      handler(f) {
        this.getItems(this.formTransfer.fromDepartmentId, f)
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    getTransferDocs(n) {
      if (this.tab === 'send') {
        if (n === 1)
          this.payload.page = n
        TransferService.getSendDocs(this.search, this.payload).then(res => {
        ...
        if (n === 1)
          this.payload.page = n
        TransferService.getReceivedDocs(this.search, this.payload).then(res => {
          this.transfers = res.data.list
          this.totalTransfer = res.data.total
        }).catch(e => {
          this.$vs.notification({
        ...
        if (n === 1)
          this.payload.page = n
        TransferService.getPendingDocs(this.search, this.payload).then(res => {
          this.transfers = res.data.list
          this.totalTransfer = res.data.total
        }).catch(e => {
          this.$vs.notification({
    ...
    getItems(depId, search) {
      WareHaseService.getItemByDepIdAndSearch(depId, search, this.pagination).then(res => {
    ...
    createForm() {
      ...
      this.getCategories()
      this.getBrands()
      TransferService.createTransferDoc().then(res => {
        this.formTransfer = {}
    ...
    viewForm(docId) {
      this.$router.push(`/wms/transfer/transfer-view/${docId}`)
    },
    editForm(docId) {
      TransferService.getTransferDocById(docId).then(res => {
        this.itemId = ''
        this.dynamicItem = {}
        this.formTransfer = res.data
    ...
    deleteTransferDoc() {
      TransferService.deleteTransferDoc(this.docId).then(res => {
        this.$vs.notification({
          type:'confirm',
          duration: 1000,
    ...
    getItemById(depId){
      this.itemId = '';
      this.getItems(depId, this.search1);
      this.formTransfer.whItems = []
    },
    addToWhList() {
        ...
        if(this.formTransfer.fromDepartmentId !== this.formTransfer.toDepartmentId) {
          ...
          let item = Object.assign({}, this.dynamicItem)
            if(item.remaining >= this.dynamicItem.count) {
              ...
              this.getItems(this.formTransfer.fromDepartmentId,'');
              this.dynamicItem = {}
              this.itemId = ''
            }
            else {
    ...
    save(status) {
      this.$refs.transferForm.validate().then(valid => {
        if (valid) {
          ...
          if (tmp) {
            TransferService.saveTransferDoc(this.formTransfer).then(res => {
    ...
    viewPendingForm(docId) {
      this.$router.push(`/wms/transfer/transfer-view/${docId}`)
    },
    filter() {
      this.getChosenItems.categoryId = this.categoryId
      this.getChosenItems.brandId = this.brandId
      this.getChosenItems.seasonsId = this.seasonsId
      this.getChosenItems.departmentId = this.depId
      WareHaseService.getChosenItems(this.getChosenItems).then(res => {
        this.formTransfer.whItems = []
        let lol = res.data.list
        this.whItemList.forEach(e => {
          lol.forEach(r => {
            if(r.id === e.id) {
              r.count = e.remaining
            }
        ...
        this.formTransfer.whItems = lol
      })
    },
    openAndClose() {
      this.transferBySomeStuffModel = !this.transferBySomeStuffModel
      this.categoryId = ''
      this.brandId = ''
      this.seasonsId = ''
    },
